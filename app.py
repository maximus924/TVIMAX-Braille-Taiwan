...
try:
    ...
    st.error("🔴 嚴重錯誤：找不到 'pypinyin' 套件！請檢查 requirements.txt。")
    st.stop()
...
class BrailleRules:
    ...
    NEMETH = {
        ...
        'OPERATION_SIGNS': {'+': '⠬', '-': '⠤', '×': '⠈⠡', '*': '⠈⠡', '÷': '⠨⠌', '/': '⠨⠌'},
        'COMPARISON_SIGNS': {'=': '⠀⠨⠅⠀', '>': '⠀⠨⠂⠀', '<': '⠀⠐⠅⠀'},
        'MATH_KEYWORDS': {'加': '+', '＋': '+', '減': '-', '－': '-', '負': '-', '乘以': '×', '乘': '×', '除以': '÷', '除': '÷', '等於': '=', '＝': '=', '：': ':', '.': '.', 'x': 'x', 'X': 'x'},
...
def text_to_braille(text, custom_rules=None, mode='UEB', use_nemeth_indicators=False):
    ...
    text_index, is_number_mode = 0, False
    ...
    while text_index < len(text):
        try: # 【關鍵修正】大迴圈防護罩
            ...
            if char == '\n':
                full_braille += "\n" # 保留換行
                dual_list.append({'char': '\n', 'braille': '', 'is_error': False})
                text_index += 1
                is_number_mode = False # 換行重置數字模式
                last_math_token = 'SPACE' # 換行視為空格
                continue
            ...
            for word, braille_code in braille_overrides.items():
                if text.startswith(word, text_index):
                    if is_nemeth_mode and use_nemeth_indicators and nemeth_context == 'MATH':
                        full_braille += rules.NEMETH['SWITCH']['END']
            ...
            if is_nemeth_mode:
                ...
                if char == ' ' or char == '　': # 包含全形空格
                    full_braille += " "
                    dual_list.append({'char': ' ', 'braille': ' ', 'is_error': False})
                    text_index += 1; last_math_token = 'SPACE'; continue

                if is_math_char:
                    if nemeth_context == 'LITERARY':
                        if use_nemeth_indicators:
                            full_braille += rules.NEMETH['SWITCH']['START']
                            dual_list.append({'char': '', 'braille': rules.NEMETH['SWITCH']['START'], 'is_error': False})
                        nemeth_context = 'MATH'; last_math_token = 'SPACE'; math_level = 0
                    
                    char_braille = ""
                    if target_char == '^':
                        char_braille = rules.NEMETH['INDICATORS']['SUPERSCRIPT']
                        full_braille += char_braille; dual_list.append({'char': char, 'braille': char_braille, 'is_error': False})
                        last_math_token = 'INDICATOR'; math_level += 1; text_index += 1; continue
                    
                    if target_char in rules.NEMETH['OPERATION_SIGNS']:
                        if math_level > 0:
                            full_braille += rules.NEMETH['INDICATORS']['BASELINE']
                            dual_list.append({'char': '', 'braille': rules.NEMETH['INDICATORS']['BASELINE'], 'is_error': False})
                            math_level = 0
            ...
            if len(current_segment) > 0 and current_segment[0].lower() in rules.ENGLISH:
                ...
                for i, c in enumerate(current_segment):
                    ...
                    full_braille += cb
                    dual_list.append({'char': c, 'braille': cb, 'is_error': False})
            ...
            if char.isdigit():
                ...
                cb += SAFE_MAP.get(char, char)
                full_braille += cb; dual_list.append({'char': char, 'braille': cb, 'is_error': False})
                text_index += 1; continue
            ...
            if char == ' ':
                is_number_mode = False
                full_braille += " "; dual_list.append({'char': ' ', 'braille': ' ', 'is_error': False})
                text_index += 1; continue

            is_number_mode = False
            single_pinyin = pinyin(char, style=Style.BOPOMOFO)
            # 防護：萬一 pypinyin 回傳怪東西
            zhuyin_data = ""
            if single_pinyin and len(single_pinyin) > 0:
                zhuyin_data = single_pinyin[0][0]
            ...
            # === 超級防護罩 ===
            # 如果單個字處理失敗，直接跳過該字，並印出錯誤 (但不當機)
            print(f"Error processing char: {text[text_index]}, Error: {e}")
            dual_list.append({'char': text[text_index], 'braille': '?', 'is_error': True})
            text_index += 1
            continue
...
def generate_html_content(dual_data, chars_per_line, font_size_px):
    ...
    for item in dual_data:
        char = item['char']
        if char == '\n': 
            html_parts.append('<div class="break-line"></div>'); current_line_len = 0
            continue
